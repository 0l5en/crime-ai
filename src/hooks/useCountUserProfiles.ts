...
export const useCountUserProfiles = (userProfileFilterDto: UserProfileFilterDto | undefined) => {
    const client = createClient<paths>({ baseUrl: PATH_CRIME_AI_API });

    return useQuery({
        queryKey: ['userProfilesCount', userProfileFilterDto],
        queryFn: async () => {
            if (!userProfileFilterDto) {
                throw new Error('userProfileFilterDto is required');
            }
            const response = await client.POST('/user-profile-count', {
                headers: {
            ...
            if (response.error) {
                throw new Error('Failed to count user profiles: ' + response.response.status);
            }
