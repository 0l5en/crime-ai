...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting prompt template request');
    ...
    console.log('Calling Crime AI API for prompt template with ID:', templateId);

    // Call the API
    const { data, error } = await client.GET("/prompt-template/{id}", {
      params: {
        path: {
          id: templateId,
        },
    ...
    console.log('Successfully fetched prompt template:', data);
