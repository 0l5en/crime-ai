...
Deno.serve(async (req) => {
  ...
  try {
    console.log('Starting prompt template versions request');
    ...
    // Parse request body to get template name
    ...
    console.log('Calling Crime AI API for prompt template versions with name:', templateName);

    // Call the API
    const { data, error } = await client.GET("/prompt-template-history", {
      params: {
        query: {
          name: templateName,
        },
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to fetch prompt template versions' }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
    ...
    console.log('Successfully fetched prompt template versions:', data);
